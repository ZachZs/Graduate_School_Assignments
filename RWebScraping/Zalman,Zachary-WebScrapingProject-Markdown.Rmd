...
This program was done as part of my studies for graduate school. The purpose of the assignment was to learn about web scraping, so I decided to take information about movies from IMBD top movies lists. It should be generalized enough to work for any list, with the only stipulation being that you have to know how many pages the code will loop through (I still haven't worked out how to bypass the error that comes up yet).
...
Inside the main for loop of the program, I scrape multiple parts from each movie (director, IMDB score, Metacritic Score, title, ranking, rating, runtime, and release year) into their own lists, then combine the lists into a dataframe. Once the dataframe is constructed, I pull various visualizations/tables to highlight some trends. The full code can be seen on my [GitHub](https://github.com/ZachZs/RWebScraping).
...
The first visualization shows the number of movies in the list for each year. As can be seen, there is an increase in number per year as we get closer to the current year.
...
This graph shows the average IMDB score and the average Metacritic score for each year. The IMDB scores were adjusted to the same scale as the Metacritic scores by multipling them by 10. Now, both scores are on a 100 scale. While the IMDB scores remain fairly constant over the years, the Metacritic scores tend to fluctuate more.
...
This graph isolates just the IMDB scores, and plots the average score for each year. While the overall trend is fairly constant, a year-to-year shift can be quite drastic.
...
This graph shows the average runtime of movies for each year. Again, the overall trend seems to be fairly consistent, with some increased fluctuation in the earlier years of film.
...
This table shows the directors with the most movies in the list.
...
knitr::kable(top_directors, caption="Directors with Most Movies")
```

The following table was built using RSelenium. I scraped Wikipedia using the directors table from before, and attached the years that the directors were active from the biography card on the Wikipedia page. For directors without a biography card, a tryCatch() function was used to add "No Information" to the table.
...
knitr::kable(top_directors, caption="Directors (Years Active")
```
